...
class RateLimiter:
    ...
    def check_message_rate(self, user_id: int) -> Tuple[bool, str]:
        ...
        if self.is_user_blocked(user_id):
            return False, "⏱️ Вы отправляете слишком много сообщений. Попробуйте через минуту."
    ...
    def cleanup(self):
        ...
        logger.debug(f"🧹 Rate limiter: очищено {len(self.message_timestamps)} пользователей в памяти")
