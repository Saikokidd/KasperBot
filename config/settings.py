...
class Settings:
    ...
    def _validate_env(self):
        ...
        if not bot_token or ":" not in bot_token:
            raise ValueError(
                "❌ Неверный формат BOT_TOKEN (должен быть вида: 123456789:ABC...)"
            )
        ...
        if not bmw_group.startswith("-"):
            raise ValueError(
                f"❌ BMW_GROUP_ID должен начинаться с '-' (сейчас: {bmw_group})"
            )

        if not zvon_group.startswith("-"):
            raise ValueError(
                f"❌ ZVONARI_GROUP_ID должен начинаться с '-' (сейчас: {zvon_group})"
            )
    ...
    def _validate_optional_env(self):
        ...
        if not self.GOOGLE_APPS_SCRIPT_URL:
            warnings.append(
                "GOOGLE_APPS_SCRIPT_URL не настроен - статистика трубок не будет работать"
            )
        elif not (
            self.GOOGLE_APPS_SCRIPT_URL.startswith("http://")
            or self.GOOGLE_APPS_SCRIPT_URL.startswith("https://")
        ):
            warnings.append(
                f"GOOGLE_APPS_SCRIPT_URL имеет неверный формат: {self.GOOGLE_APPS_SCRIPT_URL}"
            )
        ...
        if not google_sheets_id:
            warnings.append(
                "GOOGLE_SHEETS_ID не настроен - автообновление статистики не будет работать"
            )
        ...
        if not os.path.exists(google_creds):
            warnings.append(
                f"Файл {google_creds} не найден - Google Sheets интеграция не будет работать"
            )
    ...
    def _parse_admins(self):
        ...
        if admins_str:
            ...
            self.ADMINS = [
                int(id.strip()) for id in admins_str.split(",") if id.strip().isdigit()
            ]
        else:
    ...
    def _parse_pult(self):
        ...
        if pult_str:
            self.PULT = [
                int(id.strip()) for id in pult_str.split(",") if id.strip().isdigit()
            ]
        else:
            self.PULT = []
    ...
    def _parse_legacy_managers(self):
        ...
        if managers_str:
            ...
            logger.warning(
                f"   Найдено {len(self._legacy_managers)} менеджеров для миграции"
            )
    ...
    def get_telephony_groups(self) -> Dict[str, int]:
        ...
        return {"BMW": self.BMW_GROUP_ID, "Звонари": self.ZVONARI_GROUP_ID}
...
try:
    ...
    print("❌ КРИТИЧЕСКАЯ ОШИБКА: Не удалось загрузить настройки!")
    print(f"   Причина: {e}")
    print(
