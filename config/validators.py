...
class InputValidator:
    ...
    def validate_user_id(user_id: any) -> Tuple[bool, Optional[str]]:
        ...
        if user_id <= 0:
            return (
                False,
                f"❌ ID пользователя должен быть положительным числом (получено: {user_id})",
            )

        if user_id < InputValidator.MIN_USER_ID or user_id > InputValidator.MAX_USER_ID:
            return (
                False,
                f"❌ ID пользователя вне допустимого диапазона ({InputValidator.MIN_USER_ID}-{InputValidator.MAX_USER_ID})",
            )
    ...
    def validate_telephony_code(code: str) -> Tuple[bool, Optional[str]]:
        ...
        if not isinstance(code, str):
            return False, "❌ Код телефонии должен быть строкой"
        ...
        if len(code) > 50:
            return False, "❌ Код телефонии слишком длинный (макс 50 символов)"
    ...
    def validate_sip_number(sip: str) -> Tuple[bool, Optional[str]]:
        ...
        if not isinstance(sip, str):
            return False, "❌ SIP должен быть строкой"
        ...
        if len(sip) > MAX_SIP_LENGTH:
            return (
                False,
                f"❌ SIP слишком длинный (макс {MAX_SIP_LENGTH} символов, получено {len(sip)})",
            )
    ...
    def validate_telephony_name(name: str) -> Tuple[bool, Optional[str]]:
        ...
        if not isinstance(name, str):
            return False, "❌ Имя телефонии должна быть строкой"

        name = name.strip()

        if not name:
            return False, "❌ Имя телефонии не может быть пустым"

        if len(name) > 100:
            return False, "❌ Имя телефонии слишком длинное (макс 100 символов)"
    ...
    def validate_error_description(description: str) -> Tuple[bool, Optional[str]]:
        ...
        if not description:
            return False, "❌ Описание ошибки не может быть пустым"

        if len(description) > MAX_CUSTOM_ERROR_LENGTH:
            return (
                False,
                f"❌ Описание слишком длинное (макс {MAX_CUSTOM_ERROR_LENGTH} символов, получено {len(description)})",
            )
    ...
    def validate_group_id(group_id: any) -> Tuple[bool, Optional[str]]:
        ...
        if group_id > 0:
            return False, "❌ ID группы должен быть отрицательным (начинаться с '-')"
    ...
    def validate_username(username: str) -> Tuple[bool, Optional[str]]:
        ...
        if not isinstance(username, str):
            return False, "❌ Username должен быть строкой"
