...
async def handle_telephony_selection_unified(
    ...
    """
    logger.info(f"📞 Unified обработчик: выбрана телефония {tel_code} (workflow: {workflow})")
...
async def send_choose_telephony_message(
    ...
    try:
        if use_edit and update.callback_query:
            ...
            logger.error("❌ Нет способа отправить сообщение (нет callback_query и message)")
            return False
