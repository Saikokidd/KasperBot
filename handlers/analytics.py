...
def get_dashboard_navigation(page: int, period: str = "today") -> InlineKeyboardMarkup:
    ...
    if page > 1:
        nav_buttons.append(
            InlineKeyboardButton(
                "◀️ Назад", callback_data=f"dash_page_{page - 1}_{period}"
            )
        )
    if page < 4:
        nav_buttons.append(
            InlineKeyboardButton(
                "Далее ▶️", callback_data=f"dash_page_{page + 1}_{period}"
            )
        )
    ...
    page_buttons = [
        InlineKeyboardButton(
            "📊" if page == 1 else "1", callback_data=f"dash_page_1_{period}"
        ),
        InlineKeyboardButton(
            "👥" if page == 2 else "2", callback_data=f"dash_page_2_{period}"
        ),
        InlineKeyboardButton(
            "🛠" if page == 3 else "3", callback_data=f"dash_page_3_{period}"
        ),
        InlineKeyboardButton(
            "⏱" if page == 4 else "4", callback_data=f"dash_page_4_{period}"
        ),
    ]
    ...
    period_buttons = [
        InlineKeyboardButton(
            "📅" if period == "today" else "Сегодня",
            callback_data=f"dash_page_{page}_today",
        ),
        InlineKeyboardButton(
            "📆" if period == "week" else "Неделя",
            callback_data=f"dash_page_{page}_week",
        ),
        InlineKeyboardButton(
            "📊" if period == "month" else "Месяц",
            callback_data=f"dash_page_{page}_month",
        ),
    ]
    ...
    buttons.append(
        [
            InlineKeyboardButton(
                "🔄 Обновить", callback_data=f"dash_page_{page}_{period}"
            ),
...
async def show_managers_stats_period(
...
async def show_response_time_stats_period(
    update: Update, context: ContextTypes.DEFAULT_TYPE
):
    """Заглушка"""
    await show_dashboard_page(update, context)
