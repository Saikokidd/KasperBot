        ...
        [
            [
                InlineKeyboardButton(
                    "➕ Добавить менеджера", callback_data="mgmt_add_manager"
                )
            ],
            [
                InlineKeyboardButton(
                    "➖ Удалить менеджера", callback_data="mgmt_remove_manager"
                )
            ],
            [
                InlineKeyboardButton(
                    "📋 Список менеджеров", callback_data="mgmt_list_managers"
                )
            ],
...
async def add_manager_process(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    keyboard = InlineKeyboardMarkup(
        [
            [
                InlineKeyboardButton(
                    "« К управлению менеджерами", callback_data="mgmt_managers"
                )
            ]
        ]
    )
...
async def remove_manager_process(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await update.message.reply_text(
            "❌ Неверный формат! Отправьте число (ID пользователя)."
        )
        return WAITING_MANAGER_ID_REMOVE
...
async def telephonies_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    keyboard = InlineKeyboardMarkup(
        [
            [
                InlineKeyboardButton(
                    "➕ Добавить телефонию", callback_data="mgmt_add_tel"
                )
            ],
            [
                InlineKeyboardButton(
                    "➖ Удалить телефонию", callback_data="mgmt_remove_tel"
                )
            ],
...
async def add_telephony_type(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    type_name = (
        "⚪️ Белая (с кнопками)" if tel_type == "white" else "⚫️ Чёрная (без кнопок)"
    )
...
async def add_telephony_group(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    keyboard = InlineKeyboardMarkup(
        [
            [
                InlineKeyboardButton(
                    "« К управлению телефониями", callback_data="mgmt_telephonies"
                )
            ]
        ]
    )
...
async def broadcast_process(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    keyboard = InlineKeyboardMarkup(
        [
            [
                InlineKeyboardButton(
                    "✅ Да, отправить", callback_data="broadcast_confirm"
                ),
    ...
    await update.message.reply_text(
        "📨 Подтвердите отправку рассылки всем менеджерам:", reply_markup=keyboard
    )
...
async def quick_errors_add_process(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    keyboard = InlineKeyboardMarkup(
        [
            [
                InlineKeyboardButton(
                    "« К быстрым ошибкам", callback_data="mgmt_quick_errors"
                )
            ]
        ]
    )
...
async def quick_errors_remove_process(
