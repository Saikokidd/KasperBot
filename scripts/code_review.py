...
class CodeReviewer:
    ...
    def review_project(self) -> Dict[str, Any]:
        ...
        for py_file in py_files:
            ...
            if any(skip in str(py_file) for skip in ["venv", "__pycache__", ".git", "test_"]):
                continue
    ...
    def _check_exception_handling(self, file_path: Path, content: str, lines: List[str]) -> None:
        ...
        for i, line in enumerate(lines, 1):
            if re.match(try_pattern, line):
                ...
                in_try = False
            elif i > try_line + 5 and in_try and not re.match(r'^\s*(except|finally)', line):
    ...
    def _check_bare_except(self, file_path: Path, content: str, lines: List[str]) -> None:
    ...
    def _check_sql_issues(self, file_path: Path, content: str, lines: List[str]) -> None:
    ...
    def _check_missing_query_answer(self, file_path: Path, content: str, lines: List[str]) -> None:
        ...
        for i, line in enumerate(lines, 1):
            if "query.callback_query" in line or "update.callback_query" in line:
                ...
                if "query.answer()" not in snippet and "await query.answer()" not in snippet:
                    self.issues[str(file_path)].append({
                        "line": i,
                        "severity": "WARNING",
    ...
    def _check_logging_issues(self, file_path: Path, content: str, lines: List[str]) -> None:
    ...
    if results["issues"]:
        for file_path, issues in results["issues"].items():
            ...
            for issue in issues[:5]:  # Показываем первые 5 проблем
                print(f"  Линия {issue['line']}: [{issue['severity']}] {issue['message']}")
    else:
