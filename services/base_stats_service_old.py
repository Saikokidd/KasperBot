...
class BaseStatsService:
    ...
    def _count_calls_from_raw_data(
        ...
        for day_offset in range(6):  # ПН-СБ
            ...
            for provider, data in sorted(stats.items()):
                ...
                format_requests.extend(
                    self._get_provider_row_format(sheet_id, row, pct_recalls)
                )
    ...
    def _get_provider_row_format(
        self, sheet_id: int, row: int, pct_recalls: float
    ) -> List[Dict]:
