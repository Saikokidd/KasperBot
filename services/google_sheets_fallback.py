...
class GoogleSheetsFallback:
    ...
    def get_manager_stats_safe(
        ...
        try:
            self.logger.debug(f"📊 Получаем статистику менеджера {manager_id} из Google Sheets...")
            ...
            if use_cache:
                self.logger.info(f"💾 Используем кэшированную статистику для менеджера {manager_id}")
                cached_stats = sheets_cache.load_from_cache(cache_key, max_age_hours=24)
                
                if cached_stats:
                    self.logger.info(f"✅ Возвращаем кэшированную статистику")
                    return cached_stats
                else:
                    self.logger.error(f"❌ Кэш не найден для менеджера {manager_id}")
                    return None
            else:
                self.logger.error(f"❌ Не удалось получить статистику и кэш отключён")
                return None
    ...
    def get_all_managers_stats_safe(
        ...
        try:
            self.logger.debug(f"📊 Получаем статистику всех менеджеров из Google Sheets...")
            
            stats = self.service.get_all_managers_stats()
            
            # Успешно! Сохраняем в кэш
            sheets_cache.save_to_cache(cache_key, stats)
            self.logger.info(f"✅ Статистика всех менеджеров успешно получена")
            ...
            if use_cache:
                self.logger.info(f"💾 Используем кэшированную статистику всех менеджеров")
                cached_stats = sheets_cache.load_from_cache(cache_key, max_age_hours=24)
                
                if cached_stats:
                    self.logger.info(f"✅ Возвращаем кэшированную статистику ({len(cached_stats)} менеджеров)")
                    return cached_stats
                else:
                    self.logger.error(f"❌ Кэш не найден для всех менеджеров")
                    return None
            else:
                return None
    ...
    def sync_stats_safe(self) -> bool:
        ...
        """
        try:
            self.logger.debug(f"🔄 Синхронизируем статистику с Google Sheets...")
            
            result = self.service.sync_stats()
            
            self.logger.info(f"✅ Синхронизация успешна")
            return result
            
        except Exception as e:
            self.logger.warning(f"⚠️ Ошибка синхронизации: {e}")
            self.logger.warning(f"⚠️ Синхронизация будет повторена в следующем цикле")
